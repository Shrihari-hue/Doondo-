...
export const createRatingBodySchema = z.object({
  body: z
    .object({
      ...
      /** employer-plan.md §18 — Quick Work rating context, mutually exclusive with applicationId. */
      quickWorkRequestId: objectId.optional(),
    ...
    .superRefine((v, ctx) => {
      if ((v.applicationId != null) === (v.quickWorkRequestId != null)) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['applicationId'],
          message: 'Provide exactly one of applicationId or quickWorkRequestId.',
        });
      }
    }),
});
