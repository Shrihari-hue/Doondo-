...
const DIMS: Array<{ key: Dimension; label: string; icon: string; desc: string }> = [
  { key: 'punctuality', label: 'Punctuality',   icon: '⏰', desc: 'Always on time, reliable attendance' },
  { key: 'quality',     label: 'Work Quality',  icon: '🔧', desc: 'Skill, output, and attention to detail' },
  { key: 'attitude',    label: 'Attitude',       icon: '😊', desc: 'Communication, teamwork, and initiative' },
];
          ...
          <View key={dim.key} style={{
            ...
            <View style={{ flexDirection: 'row', gap: spacing.sm }}>
              {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                  <Text style={{ fontSize: 32, color: star <= ratings[dim.key] ? STAR_ON : STAR_OFF }}>★</Text>
                </Pressable>
              ))}
            </View>
